...
generate_ci_plot <- function(lambda, n = 30, LU = LU1) {
  ...
  for(i in 1:n){
    f1 <- function(lambda) qtlambda((L[i]+1)/2, lambda) - sorted_samp[i]
    f2 <- function(lambda) qtlambda((U[i]+1)/2, lambda) - sorted_samp[i]
...
ggsave("tl_|X|_ineq_lam_combined.pdf", width = 12, height = 5, dpi = 300, units = "in")

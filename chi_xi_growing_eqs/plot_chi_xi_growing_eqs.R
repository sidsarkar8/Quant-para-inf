...
chixi_CI = function(samp,L,U,cuts_comb, fig_format = "row"){
  ...
  if(fig_format == "col"){
    fig = list()
    fig[[1]] = p + ggtitle(NULL)
    fig[[2]] = q + ggtitle(NULL)
  }else{
...
for( i in 1:5)
{
  ...
  seq1 = floor(seq(1,(n-1),length = k))/n
  cuts_comb = t(combn(seq1, m = 2)) %>% as.data.frame() %>% dplyr::rename(u1 = V1, u2 = V2)
  fig_temp =  ( chixi_CI(samp,L,U,cuts_comb, fig_format = "col"))
  fig_all[[i ]] = fig_temp[[1]]
  fig_all[[i+5]] = fig_temp[[2]]
  # ggsave( paste("chi",k,".pdf",sep = ""),plot = fig,
  #         width = 10, height = 5.5, dpi = 300, units = "in" )
  ...
  if(i == 5){
    ...
    fig_temp  =  (chixi_CI(samp,L,U,cuts_comb_edge,fig_format = "col"))
    fig_all[[i ]] = fig_temp[[1]]
    fig_all[[i+5]] = fig_temp[[2]]
  }
}
...
ggarrange(plotlist = fig_all, nrow = 2, ncol = 5)

# Split plots into rows
row1 <- ggarrange(plotlist = fig_all[1:5], nrow = 1)
row2 <- ggarrange(plotlist = fig_all[6:10], nrow = 1)

# Add left-side labels
ggarrange(
  text_grob(expression(paste("CI for ", chi, " and ", xi)), rot = 90, face = "bold", size = 14),
  row1,
  text_grob(expression(paste("(", u[1], ",", u[2], ") considered")), rot = 90, face = "bold", size = 14),
  row2,
  ncol = 2,
  nrow = 2,
  widths = c(0.1, 1)
